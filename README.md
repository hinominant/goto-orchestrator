...
[simota/agent-skills](https://github.com/simota/agent-skills) の設計思想をベースに再設計。Luna 独自エージェント（CEO, Analyst, Auditor）を追加した68エージェント体制。22の共通プロトコルでエージェント間の動作を統一。
...
| **Auditor** | LROS ABSOLUTE SPEC準拠監査。全設計・実装がREADME仕様から逸脱していないかを検証 |
...
| TOOL_RISK | ツール実行前リスク分類・Hooks連携（初心者向け安全ネット） |
...
Claude Code のツール実行前にリスクレベルを自動分類し、危険な操作を事前警告する。初心者のオンボーディングに最適。

| Level | Action | Example |
|-------|--------|---------|
| HIGH | 確認ダイアログ | `rm -rf`, `git push --force`, `DROP TABLE` |
